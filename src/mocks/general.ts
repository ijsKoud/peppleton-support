export const suggestions = "788811272899919872";
export const transcripts = "793203715154378752";

export const redcross = "793939269848399873";
export const greentick = "793929362570870794";
export const prLogo = "771799330516107265";

export const pings = {
	"797618660188880927": "<@&797902390208364594> - New Question Of The Day!",
	"751889989171806259": "<@&739489234892750919> - New Event notification!",
};

export const mRole = "742791627495571596";
export const sRole = "742790430034362440";
export const botDev = "773626906800422933";
