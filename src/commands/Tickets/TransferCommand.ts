...
export default class TransferCommand extends BaseCommand {
	...
	async run(client: DiscordClient, message: Message, args: Array<string>) {
		...
		switch (type) {
			...
			// 		opener
			// 			? opener.send(
			// 					`> ⤴ | Your ticket is transferred to ${user.toString()}, you should receive a response shortly.`
			// 			  )
			// 			: "";
			// 		user.send(
			// 			`> 📨 | ${message.author.toString()} transferred ${message.channel.toString()} to you, ${opener.toString()} is waiting on a response. Send a message to the chat to start.`
			// 		);

			// 		return message.author.send(
			// 			`> ✅ | The ticket is transferred to ${user.toString()}!`
			// 		);
			// 	} catch (e) {
	...
	async handleDepartment(
		...
		try {
			...
			message.author.send(
				`> ✅ | Your ticket is transferred to ${claimer.toString()}!`
			);
			message.channel.send(
