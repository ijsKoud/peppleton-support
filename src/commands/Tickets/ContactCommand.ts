...
import { categoryId, mRole, svRole } from "../../../config/config";
...
export default class ContactCommand extends BaseCommand {
	...
	async run(client: DiscordClient, message: Message, args: Array<string>) {
		...
		if (!boolean && message.author.id !== "304986851310043136")
		...
		try {
			...
			ticketChannel.send(
				`> ℹ | This ticket was opened by ${message.author.toString()} for ${user.user.toString()} about **${reason
					.replace(/\`/g, "")
					.replace(/\*/g, "")}**.`
			);
