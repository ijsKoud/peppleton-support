...
export default class ticketChatEvent extends BaseEvent {
	...
	async run(client: DiscordClient, message: Message) {
		...
		if (
			...
			!message.content.slice(client.prefix.length).startsWith("message")
		)
			return this.handleCommands(client, message);
		if (
			spamFilter.has(message.author.id) &&
			spamFilter.get(message.author.id) === 5
		)
			return;
		...
		if (!timeouts.has(message.author.id))
			timeouts.set(
				message.author.id,
				setTimeout(() => spamFilter.delete(message.author.id), 4e3)
			);
		...
		if (message.channel.type === "dm") {
			try {
				const channel = guild.channels.cache.find(
					(c) => c.name === `${message.author.id}-ticket`
				) as TextChannel;
				if (!channel) return;
				...
				channel.send(
					`> 💬 | Reply from ${message.author.toString()}: \`\`\`${content}\`\`\`\n > ❓ | Use \`${
						client.prefix
			...
			try {
				...
				const user = client.users.cache.get(userId)
					? client.users.cache.get(userId).partial
				...
				const content = this.clean(
					message.content.length
						? message.content.length > 1500
							? message.content
									.slice(client.prefix.length + "message ".length)
									.substr(0, 1500 - 3) + "..."
							: message.content.slice(client.prefix.length + "message ".length)
						: ""
				);

				if (message.content.split(/\s+/).slice(1).length <= 0 && !files.length)
					return;
				...
				channel.send(
					`> 💬 | Reply from **${
						...
						client.prefix
					}\` if you don't want to respond with a message. \n > Check the command list for all the commands available for tickets!`,
					{
						files,
					}
				);

				client.activeTickets.set(message.channel.id, {
					reason: "",
					lastMsg: Date.now(),
				});
