...
export default class dmEvent extends BaseEvent {
	...
	async run(client: DiscordClient, message: Message) {
		...
		try {
			...
			try {
				...
				if (
					!collector.size ||
					!["1️⃣", "2️⃣"].includes(collector.first().emoji.name)
				)
					return msg.delete();
				...
				if (collector.first().emoji.name === "2️⃣") {
					if (timeouts.has(message.author.id))
						return message.channel.send(
							"> ❌ | There is a `5` seconds cooldown on this action. Please try again later."
						);
				...
				switch (type) {
					case "department":
						...
						switch (depCollector.first().emoji.name) {
							case emojiNames[0]:
								...
								depMsg.delete();
								dmChannel.send(
									`> ${qdEmoji} | You selected the **${department}**.`
								);
								break;
							case emojiNames[1]:
								...
								depMsg.delete();
								dmChannel.send(
									`> ${dsEmoji} | You selected the **${department}**.`
								);
								break;
							case emojiNames[2]:
								...
								depMsg.delete();
								dmChannel.send(
									`> ${gdEmoji} | You selected the **${department}**.`
								);
								break;
							case emojiNames[3]:
								...
								depMsg.delete();
								dmChannel.send(
									`> ${prEmoji} | You selected the **${department}**.`
								);
								break;
						}
						break;
					...
					case "extra":
						...
						const files = this.getAttachments(
							extraCollector.first().attachments
						);
			...
			const embed = new MessageEmbed()
				...
				.setDescription([
					...
					`> 🏷 | **Title/topic**: ${
						title.length > 200 ? title.substr(0, 200 - 3) + "..." : title
					}`,
					`> 📄 | **Description**: ${
						description.length > 200
							? description.substr(0, 200 - 3) + "..."
							: description
					}`,
					`> 📂 | **Extra**: ${
						extra.length > 400 ? extra.substr(0, 400 - 3) + "..." : extra
					}`,
				])
				.addField(
					"Files",
	...
	async claimingSystem(
		...
		if (!channel)
			return console.log("No ticket channel found for " + channelId);
