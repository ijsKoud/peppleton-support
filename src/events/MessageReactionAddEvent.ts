...
export default class MessageReactionAddEvent extends BaseEvent {
  ...
  async handleFeedback(client: DiscordClient, user: User, reaction: MessageReaction) {
    try {
      if (check.has(user.id)) return reaction.users.remove(user);
      const msg = await user.send(`> ${client.utils.EmojiFinder(client, 'loading').toString()} | Searching for your feedback... please wait.`);
      ...
      const feedback = data
      ?  `> ${prEmoji} | You **${state.passed}** this application session, here is your feedback: \`\`\`${(data.feedback as string).length > 850 ? (data.feedback as string).substr(0, 850 - 3) + '...' : (data.feedback as string)}\`\`\` ❓ | Questions about the feedback? Open a ticket with the topic \`feedback question\` and a staff member will help you as soon as possible.`
